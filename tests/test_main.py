...
class TestMain:
    def test_call_no_results(self, mocker):
        mocker.patch("src.tasks_from_email.imap_connect")
        mocker.patch("src.tasks_from_email.imap_search_unseen")
        mocker.patch("src.tasks_from_email.imap_close")

        src.tasks_from_email.imap_search_unseen.return_value = ("typ", [""])

        src.tasks_from_email.main()

        src.tasks_from_email.imap_connect.assert_called_once()
        src.tasks_from_email.imap_search_unseen.assert_called_once()
        src.tasks_from_email.imap_close.assert_called_once()
    ...
    def test_call_with_mocks(self, mocker, kb, email_message, create):
        ...
        mocker.patch("src.tasks_from_email.imap_search_unseen")
        mocker.patch("src.tasks_from_email.imap_close")
        mocker.patch("src.tasks_from_email.convert_to_kb_date")
        mocker.patch("src.tasks_from_email.create_user_for_sender")
        mocker.patch("src.tasks_from_email.get_task_if_subject_matches")
        mocker.patch("src.tasks_from_email.reopen_and_update")
        ...
        src.tasks_from_email.imap_connect.return_value = imap_connection
        src.tasks_from_email.imap_search_unseen.return_value = ("typ", ["a"])
        ...
        src.tasks_from_email.convert_to_kb_date.return_value = "converted-date"
        src.tasks_from_email.create_user_for_sender.return_value = 2
        kanboard.Client.return_value = kb
        kb.get_project_by_name.return_value = {"id": 1}
        if create:
            src.tasks_from_email.get_task_if_subject_matches.return_value = (None, None)
        else:
            src.tasks_from_email.get_task_if_subject_matches.return_value = (
                1,
                {"is_active": True},
            )
        ...
        src.tasks_from_email.convert_to_kb_date.assert_has_calls(
            [
                mocker.call("rfcdate"),
                mocker.call("rfcdate", 48),
            ]
        )
        ...
        src.tasks_from_email.create_user_for_sender.called_once_with(
            kb,
            "from@example.org",
        )
        kb.add_group_member.assert_not_called()
        kb.get_project_by_name.assert_called_once_with(name="Support")
        src.tasks_from_email.get_task_if_subject_matches.assert_called_once_with(
            kb,
            "ExampleHeader",
        )
        if create:
